...
class OpenBMI(BaseDataset):
    def __init__(self, master_path, pick_events=None, **kwargs):
        self.dataset_name = "OpenBMI"
        self.event_ids = {"right": 0, "left": 1, "rest":2}
        self.n_subject = 54
        self.n_session = 2
        self.phases_name = ["EEG_MI_train", "EEG_MI_test"]
        self.n_phase = 2
        self.n_trials = 100 + 50 # add rest 50 trials
        self.tmin = 0
        self.tmax = 4
        self.tmin_rest = 4
        self.sfreq = 1000
        ...
        super(OpenBMI, self).__init__(master_path, pick_events, **kwargs)
        ...
        try:
            ...
            for session in range(1, self.n_session + 1):
                for subject in range(1, self.n_subject + 1):
                    file_name = "sess{:02d}_subj{:02d}_EEG_MI.mat".format(
                        session, subject
                    )
    ...
    def convert_raw_to_zarr(self):
        ...
        if self.is_path_empty(os.path.join(dest, "x")):
            ...
            file_name = "sess{:02d}_subj{:02d}_EEG_MI.mat"
            for i, subject in enumerate(range(1, self.n_subject + 1)):
                ...
                for j, session in enumerate(range(1, self.n_session + 1)):
                    ...
                    if self.ch_names is None:
                        self.ch_names = [
                            m.item()
                            for m in mat["EEG_MI_train"][0, 0]["chan"]
                            .squeeze()
                            .tolist()
                        ]
                        
                    for k, phase in enumerate(self.phases_name):
                        trigger = mat[phase]["t"][0,0].squeeze()
                        y_dec = mat[phase]["y_dec"][0, 0].squeeze() - 1

                        x, y = [], []
                        for trial, (t, y_i) in enumerate(zip(trigger, y_dec)):
                            x.append(mat[phase]["x"][0,0][t+int(self.tmin*self.sfreq):t+int(self.tmax*self.sfreq), :])
                            y.append(y_i)
                            if trial % 2 == 0:
                                x.append(mat[phase]["x"][0,0][t+int((self.tmin_rest+self.tmin)*self.sfreq):t+int((self.tmin_rest+self.tmax)*self.sfreq), :])
                                y.append(2)
    ...
    def subject_dependent(self, subject_idx, n_splits=5, random_state=42, **kwargs):
        ...
        x = np.concatenate(self.data["x"][subject_idx, :, 0], axis=0)  # EEG_MI_train from all session
        y = np.concatenate(self.data["y"][subject_idx, :, 0], axis=0)
        x_test = np.concatenate(self.data["x"][subject_idx, :, 1], axis=0)  # EEG_MI_test from all session
